...
// ─── Home Screen ──────────────────────────────────────────────────────────────
const HomeScreen = ({ cars, expenses, loading, onSelectCar, onAddCar }) => {
  ...
  return (
    <Shell title="Tracar" action={<Btn onClick={onAddCar} small>{Icons.plus()} Add Car</Btn>}>
      {/* Summary strip */}
...
// ─── Car Detail ───────────────────────────────────────────────────────────────
const CarDetailScreen = ({ car, expenses, onBack, onAddExpense, onMarkSold, onDeleteExpense, onDeleteCar }) => {
  const carExp = getCarExpenses(expenses, car.id);
...
export default function App() {
  ...
  if (screen === "car" && selectedCar) return <CarDetailScreen car={selectedCar} expenses={expenses} onBack={() => setScreen("home")} onAddExpense={() => setScreen("add-expense")} onMarkSold={() => setScreen("sell")} onDeleteExpense={handleDeleteExpense} onDeleteCar={handleDeleteCar} />;
  return <HomeScreen cars={cars} expenses={expenses} loading={loading} onSelectCar={(car) => { setSelectedCarId(car.id); setScreen("car"); }} onAddCar={() => setScreen("add-car")} />;
}
