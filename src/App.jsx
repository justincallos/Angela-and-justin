...
const HomeScreen = ({ cars, expenses, loading, onSelectCar, onAddCar, onHistory }) => {
  ...
  // In-stock cars expenses breakdown
  const inStockCars = cars.filter(c => c.status === "instock");
  const inStockExpenses = expenses.filter(e => inStockCars.some(c => c.id === e.car_id));
  const justinInStock = inStockExpenses.filter(e => e.paid_by === "Justin").reduce((s, e) => s + Number(e.amount || 0), 0);
  const angelaInStock = inStockExpenses.filter(e => e.paid_by === "Angela").reduce((s, e) => s + Number(e.amount || 0), 0);
  const totalInStockExpenses = justinInStock + angelaInStock;
  ...
  return (
    <Shell title="Tracar" action={<div style={{display:"flex",gap:8}}><Btn onClick={onHistory} variant="outline" small>{Icons.history()} History</Btn><Btn onClick={onAddCar} small>{Icons.plus()} Add Car</Btn></div>}>
      {/* Row 1: In Stock / Sold / This Month */}
      <div style={{ display: "flex", gap: 10, marginBottom: 10 }}>
        <div style={{ flex: 1, background: "#111827", borderRadius: 14, padding: "14px 16px" }}>
          ...
          <div style={{ fontSize: 26, fontWeight: 800, color: "#fff", marginTop: 2 }}>{inStockCars.length}</div>
        </div>
        ...
        <div onClick={() => setShowMonthly(true)} style={{ flex: 2, background: monthProfit >= 0 ? "#d1fae5" : "#fee2e2", borderRadius: 14, padding: "14px 16px", cursor: "pointer", WebkitTapHighlightColor: "transparent" }}>
          ...
          <div style={{ fontSize: 11, color: monthProfit >= 0 ? "#6ee7b7" : "#fca5a5", marginTop: 1 }}>as of {new Date().toLocaleDateString("en-AU", { day: "numeric", month: "short" })} · tap</div>
        </div>
      </div>

      {/* Row 2: In-stock expenses breakdown */}
      <div style={{ background: "#fff", border: "1.5px solid #e5e7eb", borderRadius: 14, padding: "14px 16px", marginBottom: 20 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 10 }}>Expenses in Stock Cars</div>
        <div style={{ display: "flex", gap: 0 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 11, color: "#9ca3af", fontWeight: 600 }}>Justin</div>
            <div style={{ fontSize: 18, fontWeight: 800, color: "#111827", marginTop: 2 }}>{fmt(justinInStock)}</div>
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 11, color: "#9ca3af", fontWeight: 600 }}>Angela</div>
            <div style={{ fontSize: 18, fontWeight: 800, color: "#111827", marginTop: 2 }}>{fmt(angelaInStock)}</div>
          </div>
          <div style={{ flex: 1, textAlign: "right" }}>
            <div style={{ fontSize: 11, color: "#9ca3af", fontWeight: 600 }}>Total</div>
            <div style={{ fontSize: 18, fontWeight: 800, color: "#111827", marginTop: 2 }}>{fmt(totalInStockExpenses)}</div>
          </div>
        </div>
      </div>
